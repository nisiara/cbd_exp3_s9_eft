...
-- INFORME 1: RESUMEN CLIENTES POR REGION
-- Debes obtener un resumen que muestre: 
-- Las regiones, la cantidad de clientes con más de 20 años de inscripción y el total de clientes por región.
-- Pasos a seguir:
-- 1. Usar funciones de fechas para calcular el tiempo de inscripción de los clientes.
-- 2. Ordenar los resultados por la cantidad de clientes con más de 20 años de inscripción de manera ascendente

SELECT
  r.nombre_region REGION
  , COUNT( 
      CASE 
        WHEN TRUNC( MONTHS_BETWEEN( SYSDATE, c.fecha_inscripcion ) / 12 ) > 20 THEN 1 
        ELSE NULL
        END 
  ) ANTIGUEDAD_MAYOR_20
  , COUNT( c.cod_region ) TOTAL_CLIENTES_POR_REGION
FROM
  Cliente c
...
GROUP BY
  r.nombre_region
ORDER BY
  ANTIGUEDAD_MAYOR_20 ASC;


-- 3. Almacenar la consulta como una vista en la base de datos.
CREATE VIEW V_CLIENTES_POR_REGION AS 
    SELECT
      r.nombre_region REGION
      , COUNT( 
          CASE 
...
-- 4. Optimizar el acceso a los datos creando los índices:
--      IDX_REGION: para evitar un escaneo completo de la tabla CLIENTE.
--      IDX_CLI_REGION: para mejorar el rendimiento de acceso mediante el índice

CREATE INDEX IDX_REGION ON Region(nombre_region);
CREATE INDEX IDX_CLI_REGION ON Cliente(cod_region);
...
-- Debes visualizar las transacciones cuyas cuotas vencen entre junio y diciembre, mostrando la descripción de la transacción y el promedio de montos
-- Pasos a seguir:
-- 1. Ordenar los resultados por el promedio de montos de transacciones de manera ascendente.
-- 2. Generar dos soluciones:
--      Solución 1: usando un operador SET.
--      Solución 2: usando una subconsulta, cuyos resultados se almacenarán en la tabla SELECCIÓN_TIPO_TRANSACCIÓN.
-- 3. Actualizar la tasa de interés en la tabla TIPO_TRANSACCIÓN_TARJETA, aplicando una rebaja del 1% según los datos de la tabla SELECCIÓN_TIPO_TRANSACCIÓN.



--SOLUCION 1 (OPERADOR SET)
SELECT 
      TO_CHAR(SYSDATE, 'dd-mm-yyyy') FECHA
    , ttc.cod_tptran_tarjeta CODIGO
    , UPPER(ttt.nombre_tptran_tarjeta) DESCRIPCION
    , ROUND(AVG(ttc.monto_total_transaccion), 0) AS "MONTO PROMEDIO TRANSACCION"
FROM 
...
WHERE 
    ttc.nro_transaccion IN
        (SELECT nro_transaccion FROM Transaccion_Tarjeta_Cliente
        UNION
        SELECT nro_transaccion FROM Cuota_Transac_Tarjeta_Cliente )
    AND EXTRACT(MONTH FROM cttc.fecha_venc_cuota) BETWEEN 06 AND 12
GROUP BY
...
--SOLUCION 2 (SUBCONSULTA)
SELECT 
    TO_CHAR(SYSDATE, 'dd-mm-yyyy') FECHA_TRANSACCION
    , ttc.cod_tptran_tarjeta COD_TIPO_TRANSAC
    , UPPER(ttt.nombre_tptran_tarjeta) NOMBRE_TIPO_TRANSAC
    , ROUND(AVG(ttc.monto_total_transaccion), 0) MONTO_PROMEDIO
FROM 
    Transaccion_Tarjeta_Cliente ttc
    INNER JOIN Cuota_Transac_Tarjeta_Cliente cttc ON ttc.nro_tarjeta = cttc.nro_tarjeta 
    INNER JOIN Tipo_Transaccion_Tarjeta ttt ON ttc.cod_tptran_tarjeta = ttt.cod_tptran_tarjeta
...
WHERE 
    cttc.fecha_venc_cuota IN
        (SELECT fecha_venc_cuota 
            FROM Cuota_Transac_Tarjeta_Cliente 
            WHERE EXTRACT(MONTH FROM FECHA_VENC_CUOTA) BETWEEN 06 AND 12
        )
GROUP BY
    ttc.cod_tptran_tarjeta
    , ttt.nombre_tptran_tarjeta
...
-- Los datos generados por la alternativa usando SUBCONSULTA debe ser almacenada en la tabla SELECCION_TIPO_TRANSACCION.
CREATE 
    TABLE SELECCION_TIPO_TRANSACCION AS
        SELECT 
            TO_CHAR(SYSDATE, 'dd-mm-yyyy') FECHA_TRANSACCION
            , ttc.cod_tptran_tarjeta COD_TIPO_TRANSAC
            , UPPER(ttt.nombre_tptran_tarjeta) NOMBRE_TIPO_TRANSAC
        ...
        WHERE cttc.fecha_venc_cuota IN
            (SELECT fecha_venc_cuota FROM Cuota_Transac_Tarjeta_Cliente WHERE EXTRACT(MONTH FROM FECHA_VENC_CUOTA) BETWEEN 06 AND 12)
        GROUP BY
            ttc.cod_tptran_tarjeta
            , ttt.nombre_tptran_tarjeta
        ORDER BY
...
-- VERIFICAR SATOS DE LA TABLA SELECCION_TIPO_TRANSACCION RECIEN CREADA
SELECT * FROM SELECCION_TIPO_TRANSACCION;
    
-- A partir de la información generada en la tabla SELECCIÓN_TIPO_TRANSACCION,
-- se deberá actualizar la tasa de interés de la tabla TIPO_TRANSACCION_TARJETA considerando una rebaja de 1%

-- VERIFICAR QUE CODIGOS HAY QUE MODIFICAR EN LA TABLA TIPO_TRANSACCION_TARJETA
SELECT DISTINCT COD_TIPO_TRANSAC FROM SELECCION_TIPO_TRANSACCION;
    
--REVISAR LOS PORCENTAJES PREVIO A LA ACTUALIZACION
SELECT cod_tptran_tarjeta, tasaint_tptran_tarjeta FROM TIPO_TRANSACCION_TARJETA
...
WHERE cod_tptran_tarjeta IN (
        SELECT 
            DISTINCT cod_tipo_transac 
        FROM Seleccion_Tipo_Transaccion
    );    

COMMIT;

-- VERIFICAR EL CAMBIO DE PORCENTAJE
SELECT cod_tptran_tarjeta, tasaint_tptran_tarjeta FROM TIPO_TRANSACCION_TARJETA
...
El problema consiste en generar un informe que muestre las transacciones con cuotas que vencen en el segundo semestre del año, 
específicamente entre junio y diciembre. Se requiere visualizar la descripción de cada tipo de transacción y calcular 
el promedio de los montos de estas transacciones. Además, se debe actualizar la tasa de interés de los tipos de 
transacción en función de los resultados obtenidos.


2. ¿Cuál es la información significativa que necesita para resolver el problema?
Para mostrar la información es necesario tener el código de la transacción, el monto total de la transacción. Ambos datos provienen de la tabla
Transaccion_Tarjeta_Cliente. También es necesaria saber el tipo de transacción (nombre), dato que proviene de la tabla
Tipo_Transaccion_Tarjeta y la fecha en que se realizaron las transacciones (filtrar por el segundo semestre, junio - dicicembre).


3. ¿Cuál es el propósito de la solución que se requiere?
El propósito de la solución es generar un informe que permita reconocer los tipos de transacciones
que cuentan con un determinado comportamiento en término de los montos de sus transacciones y con
vencimiento de cuotas en el segundo semestre, 
Además, se busca actualizar la tasa de interés de los tipos de transacción, aplicando una rebaja del 1% a aquellos tipos
que se encuentren en el informe generado.


4. Detalle los pasos, en lenguaje natural, necesarios para construir la alternativa que usa
SUBCONSULTA.
A: Identificar que información se desplegará en el informe para determinar a que tablas es necesario acceder
B: Con las tablas identificadas establer que tipo de Join es conveniente realizar
C: Crear una subconsulta que seleccione las transacciones con cuotas cuya fecha de vencimiento esté entre junio y diciembre 
(independiente del año). para utilizarla como filtro en la sentencia WHERE
Calcular el promedio de montos: Agrupar los resultados por la descripción del tipo de transacción y calcular el promedio de los montos de las transacciones.
Almacenar los resultados: Insertar los resultados de la consulta anterior en la tabla SELECCIÓN_TIPO_TRANSACCIÓN.
D: Utilizar las clausulas de agrupación para que el informe agrupe los resultados por el promedio del monto total de las transacciones.


5. Detalle los pasos, en lenguaje natural, necesarios para construir la alternativa que usa
OPERADOR SET.
Al igual que en el informe donde se utiliza la subconsulta, el proceso comienza de la misma forma, 
identificando la información que se desplegará en el informe para determinar cuales son las tablas requridas
Esta vez no se utiliza una subconsulta para filtrar los datos por el vencimiento de cuotas en el segundo semestre.
Se utilizó la función SET con el operador UNION ya que al combinar las filas y omite los resultados duplicados.
En este caso se hizo el match por el número de transacción en las tablas Transaccion_Tarjeta_Cliente y Cuota_Transac_Tarjeta_Cliente
y luego filtrarlas por las cuotas que vences el segundo semestre
